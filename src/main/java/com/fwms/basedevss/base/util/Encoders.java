...
public class Encoders {
    ...
    public static String desDecryptFromBase64Salary(String src) {
        if (src.equals(""))
            src = "qTJsjQXM6ts=";
        byte[] data = fromBase64(src);
        return stringFromBase64(new String(desDecrypt(data, Charsets.toBytes(PASSWORD_CRYPT_KEY))));
    }
    ...
    public static String desEncryptBase64Salary(String src) {
        if (src.equals(""))
            src = "0.00";
        return toBase64(desEncrypt(Charsets.toBytes(toBase64(src)), Charsets.toBytes(PASSWORD_CRYPT_KEY)));
    }
}
